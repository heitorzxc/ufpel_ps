...
public class OR {
    ...
    public static void executar(String nixbpq, Endereco endereco, BancoRegistradores registradores, Memoria memoria){
        ...
        if(nixbpq.equals("010001")){
          System.out.println("A intruçao é OR imediato, que armazena o valor binário "+endereco.getPalavra().getEnderecoBinario()+" no acumulador (Registrador A).");         
        } else if (nixbpq.equals("110001")){

          System.out.println("A intruçao é OR direto, que armazena o valor no endereço "+endereco.getPalavra().getEnderecoBinario()+" no acumulador (Registrador A).");         
