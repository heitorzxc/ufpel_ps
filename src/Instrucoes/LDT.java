...
public class LDT {
	public static void executar(String nixbpq, Endereco endereco, BancoRegistradores registradores, Memoria memoria, Controller controller){
		...
		if(nixbpq.equals("010000")){
			controller.handleTERMINAL("A intruçao é LDT imediato, que insere o valor inteiro do binário "+endereco.getPalavra().getEnderecoBinario()+" no Registrador T.");
			...
			controller.handleTERMINAL("O valor no Registrador T será = "+dadoConvertido);
			regT.setNumeroInteiro(dadoConvertido);
		} else if (nixbpq.equals("110000")){
			controller.handleTERMINAL("A intruçao é LDT direto, que insere o valor que está no endereço "+endereco.getPalavra().getEnderecoBinario()+" no Registrador T.");
